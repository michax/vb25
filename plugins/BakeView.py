...
def write(bus):
    ...
    if VRayBake.use and VRayBake.bake_node:
        ...
        print("[DEBUG] Scene: {0}, Node: {1}".format(scene.name, VRayBake.bake_node))

        if bake_node:
            # Get data from object if available
            uv_channel = 1
            if bake_node.vray.MtlBakeStats and bake_node.vray.MtlBakeStats.use:
                uv_channel = getUVIndex(bake_node, bake_node.vray.MtlBakeStats.uv) + 1
                if uv_channel <= 0: uv_channel = 1
