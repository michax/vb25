...
def write_object(bus):
    ...
    # Do not export MtlBakeStats data to vray structure
    '''
    if VRayObject.MtlBakeStats.use:
        base_mtl= complex_material.pop()
        ma_name= complex_material[-1]
        ofile.write("\nMtlBakeStats %s {"%(ma_name))
        ofile.write("\n\tbase_mtl= %s;"%(base_mtl))
        for param in PLUGINS['MATERIAL']['MtlBakeStats'].PARAMS:
            ofile.write("\n\t%s= %s;"%(param, a(scene,getattr(VRayObject.MtlBakeStats,param))))
        ofile.write("\n}\n")
...
def run(bus):
    ...
    if bus['preview']:
        ...
        # Added because by default -frames causes addition of 0000 to file name.
        if not VRayScene.VRayBake.use:
...
def export_and_run(bus, deferred = None):
    err = write_scene(bus)

    write_settings(bus)
    close_files(bus)

    if not err and not deferred:
        run(bus)
...
def render_deferred(engine, scene, preview= None):
